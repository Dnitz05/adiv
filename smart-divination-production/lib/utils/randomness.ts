 ...
 * 
 * EDGE RUNTIME COMPATIBLE - Uses Web Crypto API for Vercel deployment
 */

import { getSecureRandom, getSecureRandomString, generateSecureRequestId } from './universalCrypto';
import { log } from './api';
...
async function generateWithRandomOrg(request: RandomnessRequest): Promise<RandomnessResponse> {
  ...
  // Accept multiple env var names to match Vercel configuration
  const apiKey =
    process.env.RANDOM_ORG_KEY ||
    process.env.RANDOM_ORG_API_KEY ||
    process.env.RANDOM_API_KEY;
...
export async function generateRandomIntegers(
  ...
  if (request.method !== 'crypto_secure' && (process.env.RANDOM_ORG_KEY || process.env.RANDOM_ORG_API_KEY) && !request.seed) {
    try {
...
/**
 * Generate random orientations for cards (independent of card selection)
 * 
 * ULTRATHINK implementation: Separates orientation randomness from card selection
 * to ensure truly random and unpredictable card orientations
 */
export async function generateRandomOrientations(options: {
  count: number;
  seed?: string;
  baseSeed?: string; // Original seed to derive from
}): Promise<RandomnessResponse> {
  const { count, seed, baseSeed } = options;
  
  // Create derived seed for orientations to ensure independence from card selection
  const orientationSeed = seed || (baseSeed ? `${baseSeed}_orientations` : undefined);
  
  return await generateRandomIntegers({
    count,
    min: 0,
    max: 1, // 0 = upright, 1 = reversed
    seed: orientationSeed,
    metadata: { 
      technique: 'tarot_orientations',
      derivedFrom: baseSeed,
      independent: true
    },
  });
}
...
export async function generateRandomCards(options: {
  ...
  const { count, seed, allowDuplicates = false, maxValue = 77 } = options;
  ...
  if (allowDuplicates) {
    ...
    return await generateTarotCards({
      count,
      allowReversed: false, // Reversal handled separately
      deckSize: maxValue + 1,
      seed,
    });
  }
...
export async function generateTarotCards(options: {
  ...
  // If we don't have enough unique cards, generate more with randomized fallback
  if (cardIndices.length < count) {
    const remainingCount = count - cardIndices.length;
    const availableCards = Array.from({ length: deckSize }, (_, i) => i)
      .filter(i => !drawnCards.has(i));
    
    // Randomized fallback selection using crypto-secure randomness
    for (let i = 0; i < remainingCount && availableCards.length > 0; i++) {
      const randomValue = getSecureRandom();
      const randomIndex = Math.floor(randomValue * availableCards.length);
      const selectedCard = availableCards.splice(randomIndex, 1)[0];
      if (selectedCard !== undefined) {
        cardIndices.push(selectedCard);
      }
    }
  }
  
  // Generate reversal values if needed
  const reversalValues = allowReversed 
    ? response.values.slice(count, count * 2)
    : [];
  
  // Encode cards with reversal information
  const encodedValues = cardIndices.map((cardIndex, position) => {
    const isReversed = allowReversed && 
      reversalValues[position % reversalValues.length] % 2 === 1;
...
export async function generateRunes(options: {
  ...
  // If we don't have enough unique runes, generate more with randomized fallback
  if (runeIndices.length < count) {
    const remainingCount = count - runeIndices.length;
    const availableRunes = Array.from({ length: runeSetSize }, (_, i) => i)
      .filter(i => !drawnRunes.has(i));
    
    // Randomized fallback selection using crypto-secure randomness
    for (let i = 0; i < remainingCount && availableRunes.length > 0; i++) {
      const randomValue = getSecureRandom();
      const randomIndex = Math.floor(randomValue * availableRunes.length);
      const selectedRune = availableRunes.splice(randomIndex, 1)[0];
      if (selectedRune !== undefined) {
        runeIndices.push(selectedRune);
      }
    }
  }
...
function generateSeedFromSignature(signature: string): string {
  // Simple hash derivation from signature for Edge Runtime compatibility
  const hash = hashString(signature + '_signature_seed');
  return hash.toString(36).substring(0, 32);
}

/**
 * Generate unique request ID - REPLACED with universalCrypto
 * Uses Edge Runtime compatible crypto generation
 */
function generateRequestId(): string {
  return generateSecureRequestId('api');
}
