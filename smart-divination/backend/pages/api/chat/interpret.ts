...
function buildInterpretationPrompt(params: {
  ...
  const promptLines = [
    ...
    'Write a flowing, narrative interpretation without section titles.',
    'Start with a brief opening, describe each card naturally with **🃏 Card Name** in bold,',
    'then synthesize the overall meaning and provide practical guidance.',
    'Keep it cohesive and avoid using headers like "Opening", "Cards", "Synthesis", or "Guidance".',
    '',
...
async function generateInterpretationFromDeepSeek(
  ...
  // TEMPORARY: Hardcoded key while debugging Vercel env var issues
  const apiKey = process.env.DEEPSEEK_API_KEY || 'sk-c31cd42fdccf4b46a383d721b996e94f';
