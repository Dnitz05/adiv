...
import { generateRandomCards, decodeRune } from '../../../lib/utils/randomness';

import { createDivinationSession } from '../../../lib/utils/supabase';

import { recordApiMetric } from '../../../lib/utils/metrics';

import type { DrawRunesRequest, Rune } from '../../../lib/types/api';

const METRICS_PATH = '/api/draw/runes';

const ALLOW_HEADER_VALUE = 'OPTIONS, POST';
const FEATURE_ENABLED = process.env.ENABLE_RUNES === 'true';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse): Promise<void> {
  ...
  if (!FEATURE_ENABLED) {
    sendJsonError(res, 503, {
      ...
      message: 'Runes are not available yet. Please try again soon.',
      requestId,
    });
  ...
  try {
    const {
      ...
      auth,
    } = await parseApiRequest(req, drawRunesRequestSchema, { requireUser: true });

    requestId = parsedRequestId;

    const requestData = parsedData as DrawRunesRequest;
    ...
    const runes: Rune[] = randomResult.values.map((encodedValue, position) => {
      const { runeIndex, isReversed } = decodeRune(encodedValue);

      const runeData = ELDER_FUTHARK[runeIndex];

      if (!runeData) {
        throw new Error(`Invalid rune index generated: ${runeIndex}`);
      }

      const reversed = requestData.allowReversed ? isReversed : false;
      ...
      return {
        ...
        meaning: reversed ? runeData.divinatory.reversed : runeData.divinatory.upright,
      };
    });
