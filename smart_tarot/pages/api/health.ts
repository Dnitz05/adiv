...
export default async function handler(req: NextRequest) {
  ...
  try {
    ...
    const services: ServiceStatus[] = [
      {
        name: 'supabase',
        status: supabaseHealth.status === 'fulfilled' && supabaseHealth.value.status === 'healthy'
          ? 'healthy' 
          : 'unhealthy',
        responseTime: supabaseHealth.status === 'fulfilled' 
          ? supabaseHealth.value.responseTime 
          : undefined,
        lastCheck: new Date().toISOString(),
        error: supabaseHealth.status === 'rejected' 
      ...
      {
        name: 'random_org',
        status: randomOrgHealth.status === 'fulfilled' && randomOrgHealth.value.status === 'healthy'
          ? 'healthy'
          : 'degraded', // Random.org failure is not critical (we have fallbacks)
        responseTime: randomOrgHealth.status === 'fulfilled'
          ? randomOrgHealth.value.responseTime
          : undefined,
        lastCheck: new Date().toISOString(),
        error: randomOrgHealth.status === 'rejected'
    ...
    const overallStatus: 'healthy' | 'degraded' | 'unhealthy' = 
      !criticalServicesHealthy ? 'unhealthy' :
      !allServicesHealthy ? 'degraded' :
      'healthy';
