 ...
 */
function validateInterpretationRequest(requestData: any): string[] {
  const errors: string[] = [];

  // Validate technique
  if (!requestData.technique) {
    errors.push('Technique is required');
  } else if (!['tarot', 'iching', 'runes'].includes(requestData.technique)) {
    errors.push(`Invalid technique: ${requestData.technique}. Must be one of: tarot, iching, runes`);
  }
  ...
  if (!requestData.results || typeof requestData.results !== 'object') {
    ...
    switch (requestData.technique) {
      case 'tarot':
        if (!requestData.results.cards || !Array.isArray(requestData.results.cards)) {
          errors.push('Tarot results must contain a cards array');
        } else if (requestData.results.cards.length === 0) {
          errors.push('At least one tarot card is required');
        }
        break;
      
      case 'runes':
        if (!requestData.results.runes || !Array.isArray(requestData.results.runes)) {
          errors.push('Runes results must contain a runes array');
        } else if (requestData.results.runes.length === 0) {
          errors.push('At least one rune is required');
        }
        break;
      
      case 'iching':
        if (!requestData.results.hexagram || typeof requestData.results.hexagram !== 'object') {
          errors.push('I Ching results must contain a hexagram object');
        } else if (!requestData.results.hexagram.number || !requestData.results.hexagram.lines) {
  ...
  if (requestData.locale && !supportedLocales.includes(requestData.locale)) {
    errors.push(`Unsupported locale: ${requestData.locale}. Supported: ${supportedLocales.join(', ')}`);
  }

  // Validate question length if provided
  if (requestData.question && typeof requestData.question === 'string' && requestData.question.length > 500) {
    errors.push('Question must be 500 characters or less');
  }

  // Validate context length if provided
  if (requestData.context && typeof requestData.context === 'string' && requestData.context.length > 1000) {
 ...
 */
function generateFollowUpQuestions(technique: DivinationTechnique, originalQuestion?: string): string[] {
  ...
  let questions = [...baseQuestions[technique]];
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-Requested-With, X-Request-ID, X-Technique');
    ...
    const interpretationData = {
      data: content,
      summary,
      confidence: Math.min(0.95, Math.max(0.60, 0.85)), // Clamp confidence
